...
import collections
from .oagcnn.data.youtube_vos_dataset import YouTubeVOSDataset
...
def save_seq_results(in_dir, seq_name,  submission_base_dir, images_dir, annotations_dir, metadata):
    
    results_dir = os.path.join(in_dir, seq_name)
    submission_dir = os.path.join(submission_base_dir, seq_name)
    ...
    for frame_name in frame_names:
        ...
        if frame_idx == 0:
            ...
            if prev_mask == True:
                ...
                num_preds = 10
            # Matriu de costos entre GT i predictions
            # En one-shot fem nomes num prediccions del numero de objectes maxim que hi ha
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Prepare results submission.')
    parser.add_argument('--inference_dir', dest='model_name', default='model')
    parser.add_argument('--out_dir', dest='prev_mask', action='store_true')
    parser.set_defaults(prev_mask=False)
    args = parser.parse_args()

    youtube_info = YouTubeVOSDataset("test", cfg, collect_info=True)
    submission_base_dir = os.path.join(args.out_dir, "Annotations")
    make_dir(submission_base_dir)
    
    for seq_name in youtube_info.metadata.keys():
        save_seq_results(args.inference_dir, seq_name, submission_base_dir, youtube_info.images_dir, youtube_info.annotations_dir,  youtube_info.metadata,)
